...
__global__ void Sgemm(
    ...
    do{
        ...
        for (int j=0;j<BLOCK_SIZE_K-1;j++)
        {
            ...
            for (int thread_y =0;thread_y<THREAD_SIZE_Y;thread_y+=4)
            {
                FETCH_FLOAT4(frag_a[(j+1)%2][thread_y]) = FETCH_FLOAT4(As[load_stage_idx][j+1][ty*THREAD_SIZE_Y+thread_y]);
            }
            #pragma unroll
            for (int thread_x=0;thread_x<THREAD_SIZE_X;thread_x+=4)
            {
                FETCH_FLOAT4(frag_b[(j+1)%2][thread_x]) = FETCH_FLOAT4(Bs[load_stage_idx][j+1][tx*THREAD_SIZE_X+thread_x]);
            }
            #pragma unroll
            for (int thread_y =0;thread_y<THREAD_SIZE_Y;thread_y++)
            {
