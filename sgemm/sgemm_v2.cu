...
)
{
    ...
    const int warps_id = tid/32;
    const int thread_in_warps = tid%32;

    const int a_tile_index  = (warps_id/4)*32+((thread_in_warps%16)/2)*4;
    const int b_tile_index  = (warps_id%4)*16+(thread_in_warps/16)*8+(thread_in_warps%2)*4;
    ...
    __syncthreads();
    FEATCH_FLOAT4(frag_A[0][0]) = FEATCH_FLOAT4(As[0][0][a_tile_index]);
    FEATCH_FLOAT4(frag_A[0][4]) = FEATCH_FLOAT4(As[0][0][a_tile_index+64]);
    
    FEATCH_FLOAT4(frag_B[0][0]) = FEATCH_FLOAT4(Bs[0][0][b_tile_index]);
    FEATCH_FLOAT4(frag_B[0][4]) = FEATCH_FLOAT4(Bs[0][0][b_tile_index+64]);
    ...
    do {
        ...
        for (int j=0;j<BK-1;j++)
        {
            ...
            FEATCH_FLOAT4(frag_A[(j+1)%2][0]) = FEATCH_FLOAT4(As[load_stage_idx][j+1][a_tile_index]);
            FEATCH_FLOAT4(frag_A[(j+1)%2][4]) = FEATCH_FLOAT4(As[load_stage_idx][j+1][a_tile_index+64]);  
            
            
            FEATCH_FLOAT4(frag_B[(j+1)%2][0]) = FEATCH_FLOAT4(Bs[load_stage_idx][j+1][b_tile_index]);
            FEATCH_FLOAT4(frag_B[(j+1)%2][4]) = FEATCH_FLOAT4(Bs[load_stage_idx][j+1][b_tile_index+64]);
        ...
        FEATCH_FLOAT4(frag_A[0][0]) = FEATCH_FLOAT4(As[load_stage_idx^1][0][a_tile_index]); 
        FEATCH_FLOAT4(frag_A[0][4]) = FEATCH_FLOAT4(As[load_stage_idx^1][0][a_tile_index+64]); 
        
        FEATCH_FLOAT4(frag_B[0][0]) = FEATCH_FLOAT4(Bs[load_stage_idx^1][0][b_tile_index]);
        FEATCH_FLOAT4(frag_B[0][4]) = FEATCH_FLOAT4(Bs[load_stage_idx^1][0][b_tile_index+64]);
        #pragma unroll
        for (int thread_y=0;thread_y<RM;thread_y++)
        {
            #pragma unroll
    ...
    for(int i=0; i<4; i++){
      FEATCH_FLOAT4(C[OFFSET(
        BM * by + a_tile_index + i,
        BN * bx + b_tile_index,
        N)]) = FEATCH_FLOAT4(res[i][0]);
    }
    for(int i=0; i<4; i++){
      FEATCH_FLOAT4(C[OFFSET(
        BM * by + a_tile_index + i,
        BN * bx + b_tile_index+64,
        N)]) = FEATCH_FLOAT4(res[i][4]);
    }
    for(int i=0; i<4; i++){
      FEATCH_FLOAT4(C[OFFSET(
        BM * by + a_tile_index + i+64,
        BN * bx + b_tile_index,
        N)]) = FEATCH_FLOAT4(res[i+4][0]);
    }
    for(int i=0; i<4; i++){
      FEATCH_FLOAT4(C[OFFSET(
        BM * by + a_tile_index + i+64,
        BN * bx + b_tile_index+64,
        N)]) = FEATCH_FLOAT4(res[i+4][4]);
    }
